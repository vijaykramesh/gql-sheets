      ...
      valueFormatter: (params) => {
        ...
        if (cellData) {
          ...
          const markdownContent = renderToStaticMarkup(
            <ReactMarkdown>{cellData.computedValue}</ReactMarkdown>,
          );
...
export function convertCellsToRowData(
  data: Cell[],
  rowCount: number,
  ...
  for (let i = 0; i <= maxRowIndex; i++) {
    ...
    for (let j = 0; j <= maxColumnIndex; j++) {
      ...
      if (
        editModeColumnAndRow &&
        editModeColumnAndRow.row === i &&
        editModeColumnAndRow.column === columnCode
      ) {
        editMode = 1;
      }
      row[columnCode] = cell
